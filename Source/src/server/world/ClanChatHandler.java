...
public class ClanChatHandler {
				...
				for (int i = 0; i < clans[clanId].members.length; i++) {
					if (clans[clanId].members[i] > 0) {
						if (Server.playerHandler.players[clans[clanId].members[i]] != null) {
							c.getPA().sendFrame126(Server.playerHandler.players[clans[clanId].members[i]].playerName,
									slotToFill);
							slotToFill++;
						}
					}
		...
		if (clans[clanId] != null) {
			for (int j = 0; j < clans[clanId].members.length; j++) {
				if (clans[clanId].members[j] <= 0) {
					...
					messageToClan(Server.playerHandler.players[playerId].playerName + " has joined the channel.",
							clanId);
	...
	public void playerMessageToClan(int playerId, String message, int clanId) {
		...
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
				...
				c.sendClan(Server.playerHandler.players[playerId].playerName, message, clans[clanId].name,
						Server.playerHandler.players[playerId].playerRights);
			}
		}
	}
	...
	public void handleLootShare(Client c, int item, int amount) {
		sendLootShareMessage(c.clanId,
				c.playerName + " has received " + amount + "x " + server.model.items.Item.getItemName(item) + ".");
	}
				...
				for (int j = 0; j < clans[c.clanId].members.length; j++) {
					...
					if (Server.playerHandler.players[clans[c.clanId].members[j]] != null) {
						...
						x.sendMessage("<col=1532693>You received " + total + " gold as your split of this drop: "
								+ amount + " x " + server.model.items.Item.getItemName(item) + ".</col>");
