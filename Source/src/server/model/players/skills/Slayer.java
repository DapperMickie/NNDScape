...
public class Slayer {
	...
	public int[] eliteTasks = { 3943, 8596, 3847, 8528, 53, 9947, 10775, 9467, 9465, 9463, 6260, 2881, 2882, 2883, 8133,
			8349 };
	...
	public void giveTask(int taskLevel) {
		...
		if (taskLevel <= 3) {
			...
			c.sendMessage("You have been assigned to kill " + c.taskAmount + " "
					+ Server.npcHandler.getNpcListName(given) + " as a slayer task.");
		} else {
			c.slayerTask = given;
			c.taskAmount = Misc.random(5) + 1;
			c.sendMessage("You have been assigned to kill " + c.taskAmount + " "
					+ Server.npcHandler.getNpcListName(given) + " as an elite slayer task.");
		}
		/*
