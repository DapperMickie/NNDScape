...
public class Commands implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		if (c.playerRights == 4 || c.playerRights == 5 || c.playerRights == 6 || c.playerRights == 7
		...
		if (playerCommand.equalsIgnoreCase("request")) {
			...
			for (int j = 0; j < Server.playerHandler.players.length; j++) {
				if (Server.playerHandler.players[j] != null) {
					...
					if (PlayerHandler.players[j].playerRights > 0 && PlayerHandler.players[j].playerRights < 8
							&& System.currentTimeMillis() - c.lastHelp > 300000) {
						c2.sendMessage("[HELP MESSAGE] <shad=15536940>" + Misc.optimizeText(c.playerName)
								+ "</shad> Has requested help.");
			...
			for (int i = 1; i < Config.MAX_PLAYERS; i++) {
				...
				if (p.playerName != null) {
					...
					c.getPA().sendFrame126(
							"@dre@" + extra + p.playerName + "@dbl@ (" + title + ") - " + p.absX + ", " + p.absY, line);
					line++;
				}
			}
				...
				for (int i = 0; i < Config.MAX_PLAYERS; i++) {
					if (Server.playerHandler.players[i] != null) {
						if (Server.playerHandler.players[i].playerName.equalsIgnoreCase(playerToBan)) {
							...
							if (c2.InDung()) {
								c.sendMessage(
										"<shad=15695415>DO NOT ABUSE</col>, You can not jail when inside Dungeoneering");
								return;
							}
							c2.Jail = false;
	...
	public void administratorCommands(Client c, String playerCommand) {
				...
				for (int i = 0; i < Config.MAX_PLAYERS; i++) {
					if (Server.playerHandler.players[i] != null) {
						if (Server.playerHandler.players[i].playerName.equalsIgnoreCase(playerToBan)) {
							Client c2 = (Client) Server.playerHandler.players[i];
							c2.BlackMarks++;
							c2.sendMessage("You've recieved a black mark from " + c.playerName + "! You now have "
									+ c2.BlackMarks + ".");
		...
		if (playerCommand.startsWith("reloadshops")) {
			...
			for (int j = 0; j < Server.playerHandler.players.length; j++) {
				if (Server.playerHandler.players[j] != null) {
					Client c2 = (Client) Server.playerHandler.players[j];
					c2.sendMessage("<shad=15695415>[News]:" + c.playerName + " " + " Has refilled the shops.</col> "
