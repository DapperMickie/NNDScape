...
public class ClickItem implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		/*
		 * if(itemId == 8007) { if(!c.InDung);
		 * c.sendMessage("Teletabbing is Disabled, Use the Teleporting Interface."); }
		 * if (itemId == 15098 && System.currentTimeMillis() - c.diceDelay >= 1200) {
		 * //Dice Bag ID if (c.clanId >= 0);
		 * Server.clanChat.playerMessageToClan(c.playerId, "I Have Rolled A "+
		 * Misc.random(100) +" On The Percentile Dice ", c.clanId);
		 * c.startAnimation(11900); c.gfx0(2075); c.diceDelay =
		 * System.currentTimeMillis(); } else { if (c.clanId != -1) return;
		 * 
		 * }
		 */
