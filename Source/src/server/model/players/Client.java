...
public class Client extends Player {
	...
	/*
	 * public void handCannonDestory() { cannonTimer = 0; int chance =
	 * playerLevel[playerFiremaking] * 5 + 25; if(specGfx) chance/=2;
	 * if(Misc.random(chance) == 1) EventManager.getSingleton().addEvent(new Event()
	 * { public void execute(EventContainer c) { if(cannonTimer <= 0) { gfx0(2140);
	 * playerEquipment[playerWeapon] = -1;
	 * sendMessage("Your hand cannon explodes!"); int damage = Misc.random(15) + 1;
	 * setHitDiff(damage); setHitUpdateRequired(true); dealDamage(Misc.random(15) +
	 * 1); updateRequired = true;
	 * getItems().sendWeapon(playerEquipment[playerWeapon],
	 * getItems().getItemName(playerEquipment[playerWeapon]));
	 * getCombat().getPlayerAnimIndex(getItems().getItemName(playerEquipment[
	 * playerWeapon]).toLowerCase()); getItems().resetBonus();
	 * getItems().getBonus(); getItems().writeBonus();
	 * getPA().requestUpdates();getOutStream().createFrame(34);
	 * getOutStream().writeWord(6); getOutStream().writeWord(1688);
	 * getOutStream().writeByte(playerWeapon); getOutStream().writeWord(0);
	 * getOutStream().writeByte(0); updateRequired = true;
	 * setAppearanceUpdateRequired(true); c.stop(); } else { cannonTimer--; } } },
	 * 500); }
	 */
	...
	public int getCombatLevel() {
		...
		if (ran > attstr) {
			...
			combatLevel = (int) (((getLevelForXP(playerXP[1])) * 0.25) + ((getLevelForXP(playerXP[3])) * 0.25)
					...
					+ ((getLevelForXP(playerXP[2])) * 0.325) + ((getLevelForXP(playerXP[22])) * 0.121212));
		}
		return combatLevel;
	}
	...
	public String[] qCS = { "Attack", "Strength", "Defence", "Ranged", "Prayer", "Magic", "Runecrafting", "Hitpoint",
			"Agility", "Herblore", "Thieving", "Crafting", "Fletching", "Slayer", "Mining", "Smithing", "Fishing",
			"Cooking", "Firemaking", "Woodcutting", "Farming" };

	public int[][] qCAB = { { 33219, 0 }, { 33219, 2 }, { 33219, 1 }, { 33219, 4 }, { 33219, 5 }, { 33219, 6 },
			{ 33219, 20 }, { 33219, 3 }, { 33219, 16 }, { 33219, 15 }, { 33219, 17 }, { 33219, 12 }, { 33219, 9 },
			{ 33219, 18 }, { 33219, 14 }, { 33219, 13 }, { 33219, 10 }, { 33219, 7 }, { 33219, 11 }, { 33219, 8 },
			{ 33219, 19 } };
	...
	public void CatchimpNpc(String npcName, int Net, int npcId, int itemId, int AmtExp, int Req, int playerId) {
		...
		if (System.currentTimeMillis() - foodDelay >= 1500) { // anti spamm
			if (playerLevel[21] >= Req) { // first we check if he's high enough to catch
				if (playerEquipment[playerWeapon] == 10010 || playerEquipment[playerWeapon] == 11259) { // player got
																										// net?
					if (playerLevel[21] + Misc.random(10) >= Misc.random(20) + Req) { // catch chance
						if (Misc.random(1000) == 1) {
							sendMessage("You caught a GIGANTIC Impling and gained triple Experience!"); // looks like
																										// player got a
																										// net
			...
			foodDelay = System.currentTimeMillis();// we use food timer but it really doesn't mather, this is just used
													// for anti-spamm :)
		}
	}
		...
		if (System.currentTimeMillis() - foodDelay >= 1500) { // anti spamm
			if (playerLevel[21] >= Req) { // first we check if he's high enough to catch
				if (playerEquipment[playerWeapon] == 10010 || playerEquipment[playerWeapon] == 11259) { // player got
					...
					if (playerLevel[21] + Misc.random(10) >= Misc.random(20) + Req) { // catch chance
						if (Misc.random(1000) == 1) {
							sendMessage("You caught a GIGANTIC butterfly and gained triple Experience!"); // looks like
																											// player
																											// got a net
							getItems().addItem(722, 1); // itemid is different so its defined in the method
							startAnimation(6999); // this always stays 6999, no need to change this
	...
	public void initialize() {
		synchronized (this) {
			...
			/*
			 * if(totalstored > 0) { Server.itemHandler.createGroundItem(storeditems[int
			 * storeditems], getX(), getY(), 1, getId()); storeditems[int storeditems] = -1;
			 * }
			 */
			if (xpLock == true) {
				sendMessage("");
	...
	public void process() {
		...
		FetchDice();
		int totalz = (getLevelForXP(playerXP[0]) + getLevelForXP(playerXP[1]) + getLevelForXP(playerXP[2])
				...
				+ getLevelForXP(playerXP[9]) + getLevelForXP(playerXP[10]) + getLevelForXP(playerXP[11])
				+ getLevelForXP(playerXP[12]) + getLevelForXP(playerXP[13]) + getLevelForXP(playerXP[14])
				+ getLevelForXP(playerXP[15]) + getLevelForXP(playerXP[16]) + getLevelForXP(playerXP[17])
		...
		if (System.currentTimeMillis() - specDelay > Config.INCREASE_SPECIAL_AMOUNT_WITH_RING
				&& playerEquipment[playerRing] == 19669) {
		...
		if (clickObjectType > 0
				&& goodDistance(objectX + objectXOffset, objectY + objectYOffset, getX(), getY(), objectDistance)) {
		...
		if ((clickNpcType > 0) && Server.npcHandler.npcs[npcClickIndex] != null) {
			if (goodDistance(getX(), getY(), Server.npcHandler.npcs[npcClickIndex].getX(),
		...
		} else if (!inWild() && !inDuelArena() && safeTimer <= 0 && !inGWD() && !inPcBoat() && !inPcGame()) { // this
																												// makes
																												// it so
																												// attack
																												// option
		...
		if (clanDice) {
			if (clanId >= 0) {
				Server.clanChat.messageToClan(
						"Clan Chat channel-mate <col=16711680>" + playerName + "</col> " + Message, clanId);
			}
		}
		cDice = 0;
	}
