...
TextDrawingArea textDrawingArea = newRegularFont;
	...
	public void init() { 
		try {
			System.out.println("NNDScape is loading..");
	...
	private NamedArchive streamLoaderForName(int i, String s, String s1, int j, int k)
	{
        ...
        try
        {
            ...
            if(abyte0 == null)
            {
                ...
                new Update(Config.getCacheURL(), "cache.zip", signlink.findcachedir());
                  abyte0 = decompressors[0].decompress(i);
			}
        } catch (Exception e) {
			e.printStackTrace();
				...
				if((j == 13 || j == 10) && inputString.length() > 0)
				{
					if(myPrivilege == 2 || server.equals("127.0.0.1")) {
						if(inputString.equals("::donate")){
							launchURL(Config.getDonateURL());
						}
						if(inputString.equals("::vote")) {
							launchURL(Config.getVotingURL());
			}
						if(inputString.equals("::register")) {
							launchURL(Config.getServerURL());
...
RSFont textDrawingArea = aTextDrawingArea_1271;
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			if(k == 6)
			{
				loginMessage1 = Config.getServerName() + " has been updated!";
...
reestablish = new Sprite("reestablish");
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading " + Config.getServerName() + ".", 30, k);
			k += 50;
		...
		if(genericLoadingError)
		{
			...
			g.drawString("To play " + Config.getServerName() + " make sure you play from", 50, 100);
		...
		if(rsAlreadyLoaded)
		{
			...
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of " + Config.getServerName() + " already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	private void drawLoginScreen(boolean flag) {
		if (normalLogin == true) {
			...
			if(loginScreenState == 0) {
				...
				i = c1 / 2 - 20;
				chatTextDrawingArea.method382(0xffff00, c / 2, "Welcome to  "+ Config.getServerName() , i, true);
				i += 30;
				int l = c / 2 - 80;
				int k1 = c1 / 2 + 20;
			...
			if(loginScreenState == 3) {
				chatTextDrawingArea.method382(0xffff00, c / 2, Config.getServerName() + " Information", c1 / 2 - 60, true);
				int k = c1 / 2 - 35;
